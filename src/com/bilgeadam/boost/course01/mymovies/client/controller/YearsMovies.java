...
public class YearsMovies {
	...
	private void retrieveMoviesByYear(int year) {
		...
		String query = "SELECT g.genre, m.name from movies m, genres g, movie_genres mg where " + "			m.year=? AND mg.movie_id=m.id AND " + "			mg.genre_id=g.id AND " + "			g.genre = ? " + "			GROUP BY g.genre, m.name";
