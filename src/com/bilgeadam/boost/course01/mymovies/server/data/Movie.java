...
public class Movie implements Externalizable {
	...
	public String toString() {
		return "Movie [imdbId=" + this.imdbId + ", titleType=" + this.titleType + ", originalTitle=" + this.originalTitle + ", adultMovie=" + this.adultMovie + ", startYear=" + this.startYear + ", endYear=" + this.endYear + ", runtimeMinutes="
				+ this.runtimeMinutes + ", genres=" + Arrays.toString(this.genres) + "]";
	}
	...
	public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException, EOFException {
		...
		for (int i = 0; i < this.genres.length; i++) {
			this.genres[i] = Genre.valueOf(in.readUTF());
		}
	}
	...
	private static String getName(String movieId) {
		return CommonData.getInstance().readMovies(movieId).getOriginalTitle();
	}
}
